...
Route::get('/bulan', function (Request $request) {
    // Bisa tangkap parameter JS versi temanmu (month) atau versi kamu (bulan)
    $bulanIndex = $request->query('bulan', $request->query('month', null));

    // Konversi 0-based (0 = Januari)
    $month = is_null($bulanIndex) ? null : ((int)$bulanIndex + 1);

    // Tahun default ke tahun sekarang jika tidak dikirim
...
Route::middleware(['auth', 'verified'])->group(function () {
    Route::get('/dashboard/hari', function () {
        return view('dashboard_hari');
    })->name('dashboard.hari');
    ...
    Route::get('/dashboard/tahun', function () {
        return view('dashboard_tahun');
    })->name('dashboard.tahun');
});
